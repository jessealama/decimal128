...
describe("divide", () => {
    let tests = {
        ...
        "infinite decimal representation": ["0.11", "0.3", "0.36666666666666666666666"],
        "many digits, few significant": ["0.00000000000000000000000000000000000001", "2", "0.000000000000000000000000000000000000005"],
        "one third": ["1", "3", "0." + "3".repeat(MAX_SIGNIFICANT_DIGITS)],
    }
    for (let [name, [a, b, c]] of Object.entries(tests)) {
        test(name, () => {
            expect(
                new Decimal128(a)
                    .divide(new Decimal128(b))
                    .toString()
            ).toStrictEqual(c);
        });
    }
