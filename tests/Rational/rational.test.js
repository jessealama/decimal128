...
describe("toFixed", () => {
    ...
    test("exactly representable", () => {
        expect(
            new Rational(JSBI.BigInt(3), JSBI.BigInt(2)).toFixed(1)
        ).toStrictEqual("1.5");
        expect(
            new Rational(JSBI.BigInt(1), JSBI.BigInt(2)).toFixed(1)
        ).toStrictEqual("0.5");
        expect(
            new Rational(JSBI.BigInt(1), JSBI.BigInt(5)).toFixed(1)
        ).toStrictEqual("0.2");
        expect(
            new Rational(JSBI.BigInt(367), JSBI.BigInt(1000)).toFixed(3)
        ).toStrictEqual("0.367");
    });
    test("more digits requested than necessary", () => {
        expect(
            new Rational(JSBI.BigInt(3), JSBI.BigInt(2)).toFixed(3)
        ).toStrictEqual("1.500");
    });
    ...
    test("representable, greater than one", () => {
        expect(
            new Rational(JSBI.BigInt(5), JSBI.BigInt(2)).toFixed(2)
        ).toStrictEqual("2.50");
    });
    test("not exactly representable, greater than one", () => {
        expect(
            new Rational(JSBI.BigInt(5), JSBI.BigInt(3)).toFixed(2)
        ).toStrictEqual("1.66");
    });
    test("non-integer value not OK", () => {
        expect(() =>
            new Rational(JSBI.BigInt(1), JSBI.BigInt(2)).toFixed(1.6)
        ).toThrow(TypeError);
    });
